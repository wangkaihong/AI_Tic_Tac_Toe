...
public class runTicTacToe {
    ...
    public void run() {
        ...
        while ((result = isEnded()) == 0) //game loop
        {
            ...
            if (turn == 1) {
                positionTicTacToe player1NextMove = ai1.myAIAlgorithm(board, 1,depth); //1 stands for player 1
                if (makeMove(player1NextMove, 1, board))
                    printBoardTicTacToe(board);
                turn = 2;
            } else if (turn == 2) {
                positionTicTacToe player2NextMove = ai2.myAIAlgorithm(board, 2,depth); //2 stands for player 2
